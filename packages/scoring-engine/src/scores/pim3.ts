...
export const pim3 = defineScore({
  ...
  changelog: [
    ...
    },
    {
      ...
      date: "2026-08-08",
      summary:
        "Three findings from the round-4 registry retrieval, all provenance. NOTHING COMPUTED CHANGED — no coefficient, input, imputation default, tier list or probability moved. THE OPEN LIVER-TRANSPLANT DECISION IS RESOLVED BY SHOWING IT CANNOT BE RESOLVED FROM THE PAPER. ANZPICR (Jan 2019) excludes patients admitted for recovery after a liver transplant done for liver failure and flags that this differs from PIM2; PICANet (v5.4, Nov 2020) includes them. ANZICS PSG and PICANet jointly supplied the derivation data, so two custodians of the same model code the same edge case oppositely and the ambiguity is downstream of Straney 2013, live in practice. The help text now states both and names which this score follows — ANZPICR, the stricter reading and the custodian of the ratio the model was built to produce — instead of asserting the exclusion as though it were uncontested. Both registries do agree that failure OF THE GRAFT qualifies. PLAUSIBILITY BOUNDS GAIN A CITABLE SOURCE: PICANet publishes per-field ranges in an expected/validation two-tier shape (SBP 20-180 with the check above 200; PaO2 22-450 mmHg, check outside 7.5-525; base excess -30 to +20, check outside -40 to +30), so where this score bounds those inputs it is no longer only an engineering limit. Recorded with its own structural negative: neither registry publishes ranges for platelets, bilirubin, creatinine or MAP because neither COLLECTS them, and that silence must not be read as a gap to fill from them. AND THE SENTINEL TRAP IS WRITTEN DOWN BEFORE IT CAN BITE: both registries use 999 for unknown on SBP, PaO2, FiO2 and base excess, ANZPICR additionally SBP 0 for cardiac arrest and 30 for shocked-and-unmeasurable, with in-registry defaults that differ between PIM2 and PIM3. Anything ingesting an export that treats 999 as a measurement is catastrophically wrong and will look plausible. This platform collects typed input and parses no export, so it is recorded rather than guarded — guard it in the same change as any import path.",
      reason: "clarification",
    },
  ],
  ipStatus: {

 ...
 *
 * CONTESTED IN BOTH DIRECTIONS, and deliberately unchanged (research note R.11).
 * 3 is the common paediatric convention (StatPearls NBK537190 / NBK534227), but
 * Palmieri et al. (ePlasty, PMC11166384, read in full) state children require
 * approximately 6 mL/kg/%TBSA and that single-figure adult formulas underestimate
 * small children while overhydrating large ones — i.e. the failure direction of a
 * single weight-linear coefficient is UNDER-resuscitation in the small child,
 * opposite to the fluid-creep hazard the rest of this file warns about. Which
 * figure to start a child on is a clinical decision; the score surfaces the
 * disagreement in `cautions` and `notes` instead of picking a side silently.
 */
...
export const burnResuscitation = defineScore({
  ...
  references: [
    ...
    },
    {
      ...
      note: "FULL TEXT READ, 2026-08-04. Cited for the contradiction it creates with the 3 mL/kg/%TBSA figure this score emits, not to settle it — two published paediatric figures differ by a factor of two, and the objection runs OPPOSITE to the fluid-creep hazard the rest of this score's text is organised around: a single weight-linear coefficient carries no baseline requirement, so it may under-resuscitate the small child before maintenance is added. Whether adding Holliday-Segar maintenance closes the gap is not established by any source read; this score's combined figure is 8.0 mL/kg/%TBSA at 8 kg, 6.2 at 25 kg and 4.9 at 60 kg (all at 20% TBSA), so it is not a constant 6 either.",
    },
    {
      citation:
  ...
  cautions: [
    ...
    defineText(
      ...
      "The 3 mL/kg/%TBSA figure shown here is contested from the OTHER side as well, and that objection is the one this page used to be missing. A paediatric review (Palmieri et al., ePlasty, read in full) states the adult formulas prescribe 2 and 4 mL/kg/%TBSA and that children require approximately 6, because a single-figure adult formula may omit maintenance and — its words — 'underestimate needs in small children and overhydrate large children'. Two published paediatric figures for the same population, differing by a factor of two. So the risk with 3 is not only over-resuscitation: in a SMALL child it may under-resuscitate before maintenance is added. Whether adding Holliday-Segar maintenance closes that gap is not settled by any source read — this score's combined figure works out at 8.0 mL/kg/%TBSA for an 8 kg child, 6.2 for a 25 kg child and 4.9 for a 60 kg child, all at 20% TBSA, so it is not a constant 6 and it does move in the direction the objection predicts. The coefficient is deliberately unchanged: which figure to start from is a clinical decision and a local-protocol one, not a calculator's.",
    ),
    defineText(

...
| E      | ~~TOTP device loss = permanent admin lockout (single-admin org)~~ **CLOSED 2026-08-07**  | Ten one-time recovery codes generated at enrolment, all present and unconsumed in production | **TM-004 resolved.** The break-glass procedure is in `docs/runbooks/deploy-production.md` — a fresh recovery code can be minted with `psql` alone (the stored hash is plain `sha256(lowercased)`, so Postgres reproduces it byte-identically), dry-run on production inside a rolled-back transaction. "Permanent lockout" was never true. The residual single point of failure is the **password**, since Argon2id cannot be recomputed on the host; losing it costs an hour of re-seeding, not the account |

### 2.3 Calculator client-side privacy guarantee (F2, P5, TB3 — the negative invariant)

The promise on every calculator (§6.4): _"Calculations run entirely in your browser. Nothing you enter is transmitted or stored."_ Compute is pure and client-side from `packages/scoring-engine`. The threat is **silent regression** — each vector below breaks the promise without any test failing:

> **This section said "fragments don't reach servers" until 2026-08-05, and it was wrong.** Browsers do not transmit a fragment; that says nothing about a script in the same document reading `location.href` and sending it itself, which is exactly what happened (TM-013 below). Note that the **Error tracking** row already had the mechanism right — "Client SDKs capture `window.location.href` **including the fragment**" — and only got the actor wrong, naming a Sentry SDK we never shipped rather than a script the CDN injects. The generalisable correction: **state the property, not the actor.** "Entered values are not in `location.href`" needs no list of who might read it.
...
| **Edge-injected script reads the URL (REALISED 2026-08-05)** | Field state was mirrored into the fragment on every keystroke, so `location.href` held the values. Cloudflare's JS Detections — injected into every page, not disableable on the Free plan — reads `document.location.href` and POSTs it as `{"lhr": …}`. Every reload, restored tab, bookmark and shared link leaked the full field set. Same-origin, so the third-party-script control never applied; the edge-script check matched `<script src>` and the injection has no `src` | **Fixed.** Values no longer enter the URL at all: an inline script lifts an incoming fragment and `replaceState`s it out during parse, before the `DOMContentLoaded` that script binds, and sharing composes a URL on demand. Asserted in `e2e/calculator-privacy.spec.ts` as a property of the URL rather than an absence of actors (**TM-013**) |

...
| --------------- | ------------------------------- | ------- | ----------------------------------------------------------- | -------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------- |
| `age_months`    | Patient age                     | number  | months (selects MAP & creatinine band)                      | 0 to **under** 216 (<18 y; newborns <37 wk excluded from derivation) | Methods: exclusion criterion is "age 18 years or older", so 216 mo is **outside**. See Limitations for how the exclusive bound is expressed. |
...
- **The age ceiling is EXCLUSIVE of 216 months, and the implementation now says so.** Leteurtre 2013's stated exclusion criterion is **"age 18 years or older"**, so the eligible domain is **[0, 216) months** — 216.0 is outside it, not the last value inside it. Round-2 sourcing (2026-08-03) confirmed the wording; earlier notes here paraphrased it as "patients >18 y excluded", which reads as admitting exactly 18.0 y and is what the code encoded. The declaration had `max: 216`, and `NumericInput.max` in `packages/scoring-engine/src/validation.ts` is **inclusive** (it rejects only `converted > input.max`) with **no exclusive-bound facility**, so exactly 18.0 years was accepted and scored — one month wrong at the top. Corrected in v1.1.0 to the largest IEEE-754 double strictly below 216, `216 − 2⁻⁴⁵ = 215.99999999999997`: 216 lies in the binade [128, 256) where adjacent doubles are 2⁻⁴⁵ ≈ 2.84e-14 apart, so no representable age lies between the declared ceiling and 216 and the closed interval [0, max] contains exactly the same inputs as the intended half-open one. The shared validator was deliberately **not** changed then — an exclusive bound is a cross-score type change with its own regression surface. **v1.1.1 (2026-08-03) makes that change and migrates onto it**: `NumericInput` now carries an optional `maxExclusive`, this input declares `max: 216, maxExclusive: 216`, and the floating-point constant is gone. The accepted set is byte-for-byte the same — the old ceiling and 216 are adjacent doubles — so no age changed status and no computed value moved; what changed is that the declaration states the criterion instead of encoding a workaround for it, and the rejection message now reads "must be at least 0 and less than 216 months" instead of naming 215.99999999999997 as an acceptable age. Phoenix, which had the same problem in the other direction (`max: 215`, rejecting a legitimate 215.5-month-old because its whole-month field could not express "under 216"), migrated onto the same facility in its v2.1.0. Pinned from both sides in `pelod2.test.ts`.
- **Derivation population:** France + Belgium, 9 PICUs, 3,671 children, median age 15.5 mo, mortality 6.0%; premature newborns admitted at birth and patients aged 18 y or older excluded. Generalizability to US/UK or high-surgical-volume PICUs was flagged by the authors as requiring verification.

...
**A CORRECTION TO THIS FILE, 2026-08-09 — the paragraph that stood here was wrong, and wrong in the more dangerous direction.**

It read: _"One figure that must never appear on this page. A review reported 'Albuali 2020, PRISM III, AUC 0.955'. That is a misreading: the full text reports PRISM III ACCURACY 95.25%, with specificity 98.51% and sensitivity 78.46% — an accuracy percentage, not an area under the curve."_ It then instructed later passes to recognise the figure and refuse it.

**Albuali 2020 reports BOTH.** Read from PMC7667207 (Albuali WH, _J Multidiscip Healthc_ 2020;13:1687-1695, PMID 33204099, DOI 10.2147/JMDH.S282108): the ROC analysis prints **AUC 0.955 (95% CI 0.924–0.986)**, and the results separately print **accuracy 95.25%, specificity 98.51%, sensitivity 78.46%** — at a _different cut-off_. The paper carries two cut-off analyses (≥12 and ≥14), which is why two similar-looking numbers coexist. The AUC is real, published, and correctly cited.

**So the review was right and this file was wrong.** The 2026-08-04 pass evidently found the accuracy sentence, saw a number close to 0.955, and concluded the AUC did not exist rather than that both did. That is a partial read presented as a refutation.

**Why this is the worse direction of error.** A note that wrongly adds a figure gets caught by the next reader who checks the source. A note that wrongly tells people to REJECT a correct figure is self-sealing: it teaches every later pass to stop looking. This one had already been carried forward once and would have been carried again.

**Independent confirmation that the figure is legitimate:** Alkhalifah 2022 (PMID 35874581), read in full, cites it in its Discussion as _"a PRISM III AUC-ROC of 0.955 (95% CI, 0.924–0.986)"_ — a peer-reviewed Saudi paper reading the same source the same way. An external review flagged that citation as a possible propagation of this file's claimed error; checking Albuali directly showed the propagation ran the other way.

**What still holds:** 0.955 appears in no shipped figure — not in `prism.ts`, not in any scoring-engine file — and none is added by this correction. Albuali is a single-centre mechanically-ventilated cohort and is not a calibration source for this calculator. What changes is only that this file no longer instructs anyone to disbelieve it.

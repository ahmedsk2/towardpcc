...
Scope of this round: the SpO₂:FiO₂ respiratory row only. Every other threshold on this page keeps the provenance its own round gave it, and the one surviving `[NEEDS SOURCE]` marker — the non-support capping rule — is **still open**; the direct read did not address it.

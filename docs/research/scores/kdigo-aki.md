...
Row 4 stands on its own: KDIGO nowhere defines anuria numerically (neither Table 2 nor the Chapter 2.1 rationale gives a mL/kg/h value or a "zero output" definition), so an anuria flag must never be converted into an **invented rate**.
...
| `anuria`       | Anuria                            | boolean     | no       | —              | no rate is invented for it                     | own Table 2 row (≥12 h ⇒ Stage 3) and, as the absence of urine is necessarily <0.5 mL/kg/h, rows 1–2 as well (6 to <12 h ⇒ Stage 1) |
...
- **OPEN ITEM — 88.42 vs 88.4 µmol/L per mg/dL.** KDIGO prints **88.4**; the implementation converts with the project-wide shared constant `CREATININE_UMOL_PER_MGDL = 88.42` in `packages/scoring-engine/src/units/concentration.ts`, derived from creatinine's molar mass of 113.12 g/mol. Both land on the same two-decimal mg/dL figure at every threshold this score prints (353.6 µmol/L → 4.00; a 26.5 µmol/L rise → 0.30), so **no KDIGO stage turns on the difference** and the score's `notes` now states 88.42 and the guideline's 88.4 side by side rather than quietly disagreeing with this page. The constant is **not** KDIGO-specific — pSOFA and PRISM use it, and the SI-canonical `UMOL_PER_L_PER_MGDL_CREATININE` in the same file is separately set to 88.4 for PELOD-2. Standardising the two is a deliberate cross-score decision with its own regression surface, **out of scope for this revision**; do not change the constant to satisfy this page.
...
- **Anuria has no numeric definition** anywhere in Table 2 or the Chapter 2.1 rationale — it is left to clinical judgement. An implementation must therefore never derive a **rate** from an anuria flag. It must equally not read that as "anuria satisfies no rate row": the absence of urine is necessarily below any positive cutoff, so anuria satisfies the <0.5 mL/kg/h rows by entailment and anuria for 6 to <12 h is Stage 1. The prohibition is on inventing a number, not on applying the inequality the word already guarantees. [NEEDS SOURCE for a numeric anuria definition — there is none in KDIGO, and none is needed for the <0.5 rows.]
